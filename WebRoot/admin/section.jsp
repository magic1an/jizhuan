			...
			<form action="Section_list.action" method="post" class="form-search" style="display: inline;">
			  ...
			  <select name="s_section.master.id"><option value="">请选择...</option>
				<c:forEach var="master" items="${masterList }">
					<option value="${master.id }" ${s_section.master.id==master.id?'selected':'' }>${master.name }</option>
				</c:forEach>
			  </select>
			  &nbsp;
							...
							<tbody>
								<c:forEach items="${sectionList }" var="section">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;">${section.master.name }</td>
			...
			<div class="modal-body">
				<form id="fm" action="Section_save.action" method="post" enctype="multipart/form-data">
					<table>
						...
						<tr>
							<td>
								<label class="control-label" for="masterName">版主：</label>
							</td>
							<td>
								<input id="masterName" type="text" name="section.master.name" onkeydown="javascript:searchUserByName(this.value)" placeholder="请输入昵称回车">
