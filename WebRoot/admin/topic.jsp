...
function modifyTopic(id,title,userId,sectionId,publishTime,modifyTime,top,good){
	 $("#myModalLabel").html("修改帖子");
	 $("#id").val(id);
	 $("#topTitle").val(title);
	 $("#topicUser").val(userId);
	 $("#topSectionId").val(sectionId);
	 $("#topicpublishTime").val(publishTime);
	 $("#topicmodifyTime").val(modifyTime);
	 $("#topicTop").val(top);
	 $("#topicGood").val(good);
}
		...
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<table cellpadding="5px;">
				<tr>
					...
					<td>发帖人:</td>
					<td><input name="s_topic.user.name" value="${s_topic.user.name }" type="text" class="input-medium search-query" placeholder="输入发帖人..." style="width: 165px;"/></td>
					<td>所属小板块:</td>
					<td>
							...
							<tbody>
								<c:forEach items="${topicList }" var="topic">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;">
										<button class="btn btn-info" type="button" data-backdrop="static" data-toggle="modal" data-target="#dlg" onclick="return modifyTopic(${topic.id},'${topic.title}','${topic.user.id }','${topic.section.id }','${topic.publishTime }','${topic.modifyTime }','${topic.top }','${topic.good }')">修改 </button>
										&nbsp;&nbsp;<button class="btn btn-danger" type="button" onclick="javascript:deleteTopic()">删除</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
					...
					<div class="modal-body">
						<form id="fm" action="">
							<table>
								<tr>
									<td>
										<label class="control-label" for="topicTitle">帖子标题：</label>
									</td>
									<td>
										 <input id="topTitle" type="text" name="topic.title" placeholder="请输入…">
									</td>
								</tr>
								<tr>
									<td>
										<label class="control-label" for="description">发帖人：</label>
									</td>
									<td>
										 <input id="topicUser" type="text" name="topic.user.id" placeholder="请输入…">
									</td>
								</tr>
								<tr>
									<td>
										<label class="control-label" for="description">所属小板块：</label>
									</td>
									
									<td>
										<select id="topSectionId" name="topic.section.id"><option value="">请选择...</option>
											<c:forEach var="section" items="${sectionList }">
								...
								<tr>
									<td>
										<label class="control-label" for="description">发帖人时间：</label>
									</td>
									<td>
										 <input id="topicpublishTime" type="text" name="topic.publishTime" placeholder="请输入…">
									</td>
								</tr>
								<tr>
									<td>
										<label class="control-label" for="description">最后修改时间：</label>
									</td>
									<td>
										 <input id="topicmodifyTime" type="text" name="topic.modifyTime" placeholder="请输入…">
									</td>
								</tr>
								<tr>
									<td>
										<label class="control-label" for="description">是否置顶：</label>
									</td>
									<td>
										 <input id="topicTop" type="text" name="topic.top" placeholder="请输入…">
									</td>
								</tr>
								<tr>
									<td>
										<label class="control-label" for="description">是否精华：</label>
									</td>
									<td>
										 <input id="topicGood" type="text" name="topic.good" placeholder="请输入…">
									</td>
								</tr>
							</table>
